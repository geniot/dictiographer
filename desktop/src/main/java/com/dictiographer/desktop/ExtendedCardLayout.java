...
public class ExtendedCardLayout implements LayoutManager2,
        Serializable {
     ...
     */
    Vector<ExtendedCardLayout.Card> vector = new Vector<>();

    public Component getPanelByName(String name) {
        int ncomponents = vector.size();
        for (int i = 0; i < ncomponents; i++) {
            ExtendedCardLayout.Card card = vector.get(i);
     ...
     */
    public ExtendedCardLayout(int hgap, int vgap) {
        this.hgap = hgap;
        this.vgap = vgap;
    }
    ...
    public void addLayoutComponent(String name, Component comp) {
        synchronized (comp.getTreeLock()) {
            ...
            for (int i = 0; i < vector.size(); i++) {
                if (((ExtendedCardLayout.Card) vector.get(i)).name.equals(name)) {
                    ((ExtendedCardLayout.Card) vector.get(i)).comp = comp;
                    return;
                }
            }
            vector.add(new ExtendedCardLayout.Card(name, comp));
        }
    }
    ...
    public void removeLayoutComponent(Component comp) {
        synchronized (comp.getTreeLock()) {
            for (int i = 0; i < vector.size(); i++) {
                if (((ExtendedCardLayout.Card) vector.get(i)).comp == comp) {
    ...
    public void show(Container parent, String name) {
        synchronized (parent.getTreeLock()) {
            ...
            int ncomponents = vector.size();
            for (int i = 0; i < ncomponents; i++) {
                ExtendedCardLayout.Card card = (ExtendedCardLayout.Card) vector.get(i);
                if (card.name.equals(name)) {
                    next = card.comp;
                    currentCard = i;
                    break;
        ...
        if (f.defaulted("vector")) {
            ...
            if (tab != null && !tab.isEmpty()) {
                for (Enumeration<String> e = tab.keys(); e.hasMoreElements(); ) {
                    ...
                    vector.add(new ExtendedCardLayout.Card(key, comp));
                    if (comp.isVisible()) {
